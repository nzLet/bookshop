		...
		if(quantity == 1) {
			if(confirm("您是否真要删除该条目？")) {
				$.ajax({
					async:false,
					cache:false,
					url:"/bookshop/front/cartitem/delete",
					data:{cartItemId:id},
					type:"POST",
					dataType:"json",
					success:function(result) {
						if(result){
							alert("删除成功!");
							location = "/bookshop/front/cartitem/show";
						}else{
							alert("删除失败！");
						}
					}
...
function sendUpdateQuantity(id, quantity) {
	$.ajax({
		async:false,
		cache:false,
		url:"/bookshop/front/cartitem/updateNum",
...
function batchDelete() {
	...
	location = "/bookshop/front/cartitem/deleteMore/" + cartItemIdArray;
}
...
<p style="font-weight: 900; color: red">${msg }</p>
	<c:choose>
		...
		<c:otherwise>
			<table width="95%" align="center" cellpadding="0" cellspacing="0">
				...
				<c:forEach items="${cartitems }" var="cartItem">
					<tr align="center">
						...
						<td align="left" width="70px"><a class="linkImage"
							href="<c:url value='/front/book/detail/${cartItem.book.bid }'/>"><img border="0"
								width="54" align="top"
								src="<c:url value='/${cartItem.book.image_b }'/>" /></a></td>
						<td align="left" width="400px"><a
							href="<c:url value='/front/book/detail/${cartItem.book.bid }'/>"><span>${cartItem.book.bname }</span></a>
						</td>
						...
						<td><a href="#" onclick="deleteOne()" id="deleteOne" name="${cartItem.cartItemId}">删除</a>
						</td>
					</tr>
				</c:forEach>
				...
				<tr>
					<td colspan="4" class="tdBatchDelete"><a
						href="#" onclick="batchDelete()">批量删除</a></td>
			...
			<form id="jieSuanForm" action="<c:url value='/front/cartitem/loadCartItems'/>"
				method="post">
